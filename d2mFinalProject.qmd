...
Participants’ eyes are tracked during the experiment using an EyeLink 1000 Plus Eye Tracker (SR Research, Ontario, Canada). The eye-tracking device is utilized to confirm participants’ attention and record the eye-tracking measures: fixations and saccades. Fixations are instances where the eye sustains its gaze on a specific item or location (Laan et al., 2015), while a saccade is a rapid eye movement that shifts the gaze between two fixation points (Ross et al., 2001).  The total fixation duration is calculated as the sum of the duration of all fixations within the window, indicating the stimuli’s onset until the participant’s response is recorded.
...
newset <- newset %>%
  mutate(across(
    c(maxMoral, Moral_left, Moral_right, Support_left, Support_right,
      ...
      supportUnchosenReAdj, supportDiffReAdj, , consistent,      MoralDiff_Consistent,edu,gender,partyAlignment,politicalEngagement,chosenMoral,chosenFamiliar,DUREL_sum),
    ~ scale(.), 
    .names = "{.col}_scaled"
  ))
...
```{r Corr-LMER-plot,fig.cap="Effect of Familiarity of an Issue on its Moral Convicvtion"}
p_corr <- plot_model(
  CorrelationLMERTest,
  type = "pred",
...
In running a linear mixed-effects regression model, the familiarity of the sociopolitical issue was the strongest positive predictor of moral convictions ($\beta=$ `r round(summary(CorrelationLMERTest)$coefficients[3,1],3)`, p`r if(Familiar_pvalue<0.001) "< 0.001" else paste0("=",Familiar_pvalue)`). On the other hand , the support variable was `r if(Support_pvalue<0.05) "significant" else "not significant"` in predicting moral convictions ($\beta=$`r round(Support_coefficient,3)`, p = `r round(Support_pvalue,3)`).This suggest that the repeated exposure to an issue shapes the intensity of the moral beliefs more so than the political support for this issue. Moral convictions and familiarity were also `r if(corr_estimate>0 & corr_pvalue<0.05) "positively correlated" else if (corr_estimate<0 & corr_pvalue<0.05) "negatively correlated" else "not correlated"` (`r round(corr_estimate,3)`, p `r if(corr_pvalue<0.001) "<0.001" else "=round(corr_pvalue,3)"`). Other demographic factors , including religiosity, did not significantly predict moral conviction scores. 

```{r Correlation-plot, echo= FALSE,fig.cap="Count tileplot highlighting the correlation between Moral Conviction and Familiarity of Topic"}
library(ggplot2)
...
ggplot(heatmap_data, aes(x = factor(Familiar1_scaled), y = factor(Moral_left), fill = n)) +
  ...
  labs(x = "Familiar Score", y = "Moral Conviction Score", fill = "Count") +
  theme_minimal()

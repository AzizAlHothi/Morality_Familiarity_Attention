...
Familiarleft_coefficient=summary(RTLMERTest)$coefficients[6,1]
Familiarleft_pvalue=summary(RTLMERTest)$coefficients[6,5]
Familiarright_coefficient=summary(RTLMERTest)$coefficients[7,1]
Familiarright_pvalue=summary(RTLMERTest)$coefficients[7,5]
...
\mathbf{Consistent\ Item\ Moral\ Conviction\ Score} -
\mathbf{Inconsitent\ Item\ Moral\ Conviction\ Score}
\end{array}
$$ 
\vspace{1pt}
...
RT_model_results <- tidy(RTLMERTest, effects = "fixed") %>%
...
kable(RT_model_results, booktabs = TRUE) %>%
  kable_styling(latex_options = c("HOLD_position", "striped"))


```

Topic familiarity , as well as the difference in moral convictions as well as the difference in support between the two images of protestors, were found to be stronger predictors of response time. However , as @tbl-RTTable demonstrates the strongest predictor was the variable "protest indicator", which indicates the "thumbs-up" and "thumbs-down" symbols shown next to the topics`r statpresent(ProtestView,RTLMERTest)`. Protest indication was measured binarily, where +1 indicated a thumbs-up, and a -1 indicated a thumbs-down. The linear regression model indicates that when the protest indicator was a thumbs up, participants were able to respond significantly faster than the other way around. The second strongest predictor was the different in political support `r statpresent(supportDiffReAdj_scaled,RTLMERTest)`. Familiarity of the topics was significant in three different ways: the familiarity of the image on the right `r statpresent(Familiar2_scaled,RTLMERTest)`, the familiarity of the image on the left `r statpresent(Familiar1_scaled,RTLMERTest)`, and the difference of familiarity between the two images `r statpresent(FamiliarDiff_Consistent_scaled,RTLMERTest)`.
...
```{r TFD-test,echo=FALSE,include=FALSE}
#| label: TFD-test

...
newset<-newset %>%  #introducing two new variables called MoralDiff_consistent and FamiliarDiff_Consistent, which will be outlined in the manuscript. 
  mutate(MoralDiff_Consistent= case_when((consistent==1&LeftChoice==1)|(consistent==0&LeftChoice==0)~                                              Moral_left - Moral_right,
         ...
         Topic.selected=as.factor(if_else(LeftChoice==1,"Left Topic","Right Topic")),
         across(c(Right_Fixation,Right_nFixation,Unchosen_nFixations,Unchosen_Fixation,Chosen_Fixation,Chosen_nFixations,Left_nFixations,Left_Fixation,Total_nFixations),as.numeric))%>% 
  ...
  unite(combined.issues,Issue_left,Issue_right,sep = " vs. ")%>% #combining all the issues in one column using. a tidyr funciton
...
fixation_data <- newset %>%
  select(SubjectNumber, Left_Fixation, Right_Fixation) %>%
  pivot_longer(cols = c(Left_Fixation, Right_Fixation), 
               names_to = "Topic_Side", values_to = "Fixation_Duration") %>%
  mutate(Topic_Side = if_else(Topic_Side == "Left_Fixation", "Left Topic", "Right Topic"))  # Rename for clarity

# Use a more robust approach with just geom_smooth directly on binned data
# First, create bins manually
binwidth <- 100
fixation_bins <- fixation_data %>%
  mutate(bin = floor(Fixation_Duration / binwidth) * binwidth) %>%
  count(Topic_Side, bin) %>%
  # Ensure both sides have the same bins for smoother comparison
  complete(Topic_Side, bin = seq(min(bin), max(bin), by = binwidth), fill = list(n = 0))

# Now plot with smooth lines
ggplot(fixation_bins, aes(x = bin, y = n, color = Topic_Side)) +
  ...
  scale_color_manual(values = c("Left Topic" = "steelblue", "Right Topic" = "orange")) +
  theme_minimal() +
  theme(
    legend.position = "top",
...
#in this chunk , I will setup a one way anova test to determine if choosing the left topic correlated with a faster response time. 
...
```
As @tbl-Left-Right-General-Results highlight, the average fixation duration and number of fixations across the left and right topics were fairly close across all trials. Where @fig-densityplot further illustrates the closeness of the eye-tracking values across the two opposing topics across trials. Through an one-way ANOVA test , it was confirmed that the selection of a topic on the left or right did not influence the response time in a meaningful way (`r anova_print`).
